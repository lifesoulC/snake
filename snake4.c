...
{  
    ...
    for(i = 0; i < LINES; i++)  
	  ...
	  {  
	     ...
	     mvaddstr(1, 2, "Usage: Press 'f' to speed up, 's' to speed down,'q' to quit.");  
		 mvaddstr(2, 2, "       Nagivation key controls snake moving.");  
		 refresh();  
}  
    ...
    signal(SIGALRM, SIG_IGN);  
	    ...
	    if((head->next->x_pos == RIGHT_EDGE-1 && x_dir == 1)   
		   ...
		   || (head->next->y_pos == BOT_ROW-1 && y_dir == 1))  
		    ...
		    if(mvinch(head->next->y_pos + y_dir, head->next->x_pos + x_dir) == ACS_CKBOARD)  
				        gameover(2);  
